...
func TestGenerateChunkFile(t *testing.T) {
	fileInfo, err := os.Stat("tmp.jpg")
	if err != nil {
		t.Fatal(err)
	}
	// 分片的个数
	chunkNum := math.Ceil(float64(fileInfo.Size()) / float64(chunkSize))
	myFile, err := os.OpenFile("tmp.jpg", os.O_RDONLY, 0666)
	if err != nil {
		t.Fatal(err)
	}
...
func TestMergeChunkFile(t *testing.T) {
	myFile, err := os.OpenFile("res.jpg", os.O_CREATE|os.O_WRONLY|os.O_APPEND, os.ModePerm)
	if err != nil {
		t.Fatal(err)
	}
	fileInfo, err := os.Stat("tmp.jpg")
...
func TestCheck(t *testing.T) {
	// 获取第一个文件的信息
	file1, err := os.OpenFile("./tmp.jpg", os.O_RDONLY, 0666)
	if err != nil {
		t.Fatal(err)
	}
	...
	// 获取第二个文件的信息
	file2, err := os.OpenFile("res.jpg", os.O_RDONLY, 0666)
	if err != nil {
		t.Fatal(err)
	}
